...
inline float cross_value_value(const Vector3f& u, const Vector3f& v)
{
...
struct Vector4f
{
    ...
    float& operator[](int i){
        return const_cast<float&>(static_cast<const Vector4f&>(*this)[i]);
    }
...
inline Vector4f normalize(const Vector4f& u)
{
    float len = length(u);
    return(u / len);
}

inline Vector3f cross(const Vector4f& u, const Vector4f& v)
{
    Vector3f vv;
    vv.x = u.y*v.z - v.y*u.z;
    vv.y = -1 * (u.x*v.z - v.x*u.z);
...
inline float cross_value_value(const Vector4f& u, const Vector4f& v)
{
    return(u.y*v.z - v.y*u.z + -1 * (u.x*v.z - v.x*u.z) + u.x*v.y - v.x*u.y);
